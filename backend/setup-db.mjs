...
const statements = [
  ...
  `CREATE TABLE IF NOT EXISTS users (
    id SERIAL PRIMARY KEY, full_name VARCHAR(120) NOT NULL, email VARCHAR(255) NOT NULL UNIQUE,
    phone VARCHAR(30) NOT NULL, password TEXT NOT NULL, role user_role NOT NULL,
  ...
  `CREATE TABLE IF NOT EXISTS professionals (
    ...
    service_category VARCHAR(100) NOT NULL, hourly_rate VARCHAR(50) NOT NULL,
    rating NUMERIC(3,2) NOT NULL DEFAULT 0, reviews_count INTEGER NOT NULL DEFAULT 0,
    verified BOOLEAN NOT NULL DEFAULT FALSE, response_time VARCHAR(50),
  ...
  `CREATE TABLE IF NOT EXISTS services (
    id SERIAL PRIMARY KEY, name VARCHAR(120) NOT NULL UNIQUE, category VARCHAR(100) NOT NULL,
    description TEXT, created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
  )`,
  `CREATE INDEX IF NOT EXISTS idx_services_category ON services(category)`,
  `CREATE TABLE IF NOT EXISTS bookings (
    id SERIAL PRIMARY KEY, customer_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    professional_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    service VARCHAR(120) NOT NULL, date DATE NOT NULL, time TIME NOT NULL,
    location VARCHAR(255) NOT NULL, amount NUMERIC(12,2) NOT NULL,
  ...
  `CREATE TABLE IF NOT EXISTS jobs (
    id SERIAL PRIMARY KEY, customer_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    title VARCHAR(200) NOT NULL, description TEXT NOT NULL, category VARCHAR(100) NOT NULL,
    budget VARCHAR(50) NOT NULL, location VARCHAR(255) NOT NULL,
    status job_status NOT NULL DEFAULT 'active',
    proposals_count INTEGER NOT NULL DEFAULT 0, views_count INTEGER NOT NULL DEFAULT 0,
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
  )`,
  ...
  `CREATE TABLE IF NOT EXISTS conversations (
    ...
    unread_customer INTEGER NOT NULL DEFAULT 0, unread_professional INTEGER NOT NULL DEFAULT 0,
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  ...
  `CREATE TABLE IF NOT EXISTS payment_methods (
    id SERIAL PRIMARY KEY, user_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    type VARCHAR(50) NOT NULL, number VARCHAR(30) NOT NULL,
    is_default BOOLEAN NOT NULL DEFAULT FALSE, expiry VARCHAR(10),
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
  )`,
  `CREATE INDEX IF NOT EXISTS idx_payment_methods_user_id ON payment_methods(user_id)`,
  `CREATE TABLE IF NOT EXISTS transactions (
    id SERIAL PRIMARY KEY, user_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    description TEXT NOT NULL, amount NUMERIC(12,2) NOT NULL, date DATE NOT NULL,
    status tx_status NOT NULL DEFAULT 'completed', method VARCHAR(50) NOT NULL,
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
  )`,
  `CREATE INDEX IF NOT EXISTS idx_transactions_user_id ON transactions(user_id)`,
  `CREATE TABLE IF NOT EXISTS invoices (
    id SERIAL PRIMARY KEY, user_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    number VARCHAR(50) NOT NULL UNIQUE, description TEXT NOT NULL,
    amount NUMERIC(12,2) NOT NULL, date DATE NOT NULL, due_date DATE NOT NULL,
  ...
  `CREATE OR REPLACE FUNCTION set_updated_at() RETURNS TRIGGER LANGUAGE plpgsql AS $f$
  ...
  `DO $d$ DECLARE t TEXT;
   BEGIN
     FOREACH t IN ARRAY ARRAY['users','professionals','bookings','jobs','conversations','invoices'] LOOP
       EXECUTE format('CREATE OR REPLACE TRIGGER trg_%s_updated_at BEFORE UPDATE ON %s FOR EACH ROW EXECUTE FUNCTION set_updated_at();', t, t);
     END LOOP;
   END $d$`,
];
